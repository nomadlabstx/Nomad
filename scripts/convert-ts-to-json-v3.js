...
// Convert TypeScript object to JSON using vm
function convertTSToJSON(tsCode) {
  try {
    ...
    // Use vm.runInNewContext to safely evaluate
    const context = { console: { log: () => {}, error: () => {} } };
    const result = vm.runInNewContext(`(${tsCode})`, context);
    return result;
  } catch (e) {
    // Fallback: try with Function constructor but strip problematic parts
    try {
