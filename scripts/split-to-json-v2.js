...
// Use a safer approach: Evaluate the TypeScript data in a controlled way
// First, let's extract just the data part and evaluate it
let dataContent = dataMatch[1].trim();

// Function to safely parse TypeScript object literal to JSON
function parseTSObjectToJSON(tsCode) {
  // Remove trailing commas first (but be careful with strings)
  ...
  try {
    ...
    const func = new Function(`return ${tsCode}`);
    return func();
  } catch (e) {
    // Fallback: try to manually convert common patterns
...
// Function to extract state object string
